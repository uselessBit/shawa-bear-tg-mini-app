...
async def add_item(
        user_id: int, item_data: BasketItemCreate, session: AsyncSession = Depends(get_session)
) -> dict:
...
async def remove_item(basket_item_id: int, session: AsyncSession = Depends(get_session)) -> dict:
