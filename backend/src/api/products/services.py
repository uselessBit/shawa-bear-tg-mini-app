    ...
    async def create(
        ...
        async with session.begin():
            ...
            if len(ingredients) != len(product_data.ingredient_ids):
                ...
                raise HTTPException(
                    status_code=404,
                    detail=f"Ingredients not found with ids: {missing_ids}",
                )
        ...
        for ingredient_id in product_data.ingredient_ids:
            await ProductIngredientService.create(
        ...
        async with session.begin():
            ...
            if product:
                ...
                if product_data.ingredient_ids:
                    await ProductIngredientService.update(
    ...
    async def create(price_data: PriceCreate, session: AsyncSession) -> JSONResponse:
        async with session.begin():
            ...
            if not size:
                HTTPException(
                    status_code=404,
                    detail=f"Size with id {price_data.size_id} not found",
                )
